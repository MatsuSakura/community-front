  ...
  <el-main>
    ...
    <el-form :model="parms" ref="searchForm" label-width="80px" :inline="true" size="small">
      ...
      <el-form-item label="工作状态">
        <el-select v-model="parms.status" placeholder="请选择">
          <el-option v-for="item in options" label-width="80px" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-table :height="tableHeight" :data="tableList" border stripe>
      ...
      <el-table-column prop="status" label="工作状态">
        <template slot-scope="scope">
          <el-tag type="success" size="normal" v-if="scope.row.status == '0'">外出工作</el-tag>
          <el-tag type="danger" size="normal" v-if="scope.row.status == '1'">本地工作</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" width="500" label="操作">
        <template slot-scope="scope">
          <el-button type="success" size="small" @click="editBtn(scope.row)">编辑</el-button>
          <el-button size="small" @click="assignHelp(scope.row)">人文关怀</el-button>
    ...
    <el-pagination @size-change="sizeChange" @current-change="currentChange" :current-page.sync="parms.currentPage"
      :page-sizes="[5, 10, 15, 20, 30]" :page-size="parms.pageSize" layout="total, sizes, prev, pager, next, jumper"
      ...
      <template slot="content">
        <el-form :model="addModel" ref="addForm" :rules="rules" label-width="80px" :inline="true" size="small">
          ...
          <el-form-item prop="workUnit" label="工作单位">
            <el-input v-model="addModel.workUnit"></el-input>
          </el-form-item>
        ...
        <el-main style="padding: 0px">
          ...
          <el-form :model="houseParms" ref="assignHoseForm" label-width="70px" :inline="true" size="small">
            <el-form-item label="楼栋名称">
              <el-input placeholder="请输入楼栋名称" v-model="houseParms.buildNme"></el-input>
            </el-form-item>
            ...
            <el-form-item>
              <el-button icon="el-icon-search" @click="assignHouseSearchBtn">查询</el-button>
              <el-button icon="el-icon-delete" @click="assignHouseResetBtn">重置</el-button>
            </el-form-item>
          </el-form>
          <!-- 表格 -->
          <el-table :height="assignTableHeight" :data="houseList" border stripe>
            <el-table-column width="50" align="center" label="选择">
              <template slot-scope="scope">
                <el-radio v-model="assignHoseParm.houseId" :label="scope.row.houseId" @change="getCurrentRow(scope.row)">
                  {{ "" }}
                </el-radio>
              </template>
            </el-table-column>
            <el-table-column align="center" prop="name" label="楼栋名称"></el-table-column>
          ...
          <el-pagination @size-change="assignHouseSize" @current-change="assignHouseChange"
            :current-page.sync="houseParms.currentPage" :page-sizes="[5, 10, 15, 20, 30]" :page-size="houseParms.pageSize"
            layout="total, sizes, prev, pager, next, jumper" :total="houseParms.total" background>
          </el-pagination>
        </el-main>
      </template>
    </sys-dialog>
    <!-- 新增弹框 -->
    <sys-dialog :title="HelpDialog.title" :height="HelpDialog.height" :width="HelpDialog.width"
      :visible="HelpDialog.visible" @onClose="HelpClose" @onConfirm="HelpConfirm">
      <template slot="content">
        <el-form :model="addModel" ref="addForm" :rules="rules" label-width="100px" :inline="false" size="small">
